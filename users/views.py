...
def user_preferences(request):
    ...
    try:
        ...
        return JsonResponse(
            {"status": "error", "message": f"Error updating preferences: {str(e)}"}
        )
...
def setup_consumer_interests(user, selected_categories):
    ...
    interests = ConsumerInterest.objects.create(user=user, receive_notifications=True)

    # Add selected categories
    for category_name in selected_categories:
        category, _ = Category.objects.get_or_create(
            name=category_name.title(), slug=category_name.lower().replace(" ", "-")
        )
...
def get_dashboard_stats(user):
    ...
    try:
        ...
        matching_offers = (
            active_offers.filter(categories__in=preferred_categories).distinct().count()
...
def get_user_profile_data(user):
    ...
    return {"address_number": address_number, "address_street": address_street}
...
def format_offer_data(offer, today, preferred_categories):
    ...
    business_info = {"name": "Business Name", "email": "", "phone": "", "address": ""}

    if hasattr(offer, "user") and offer.user:
        business_info.update(
            {
                "name": offer.user.business_name
                or f"{offer.user.first_name} {offer.user.last_name}",
    ...
    return {
        ...
        "categories_match_preferences": (
            any(cat in preferred_categories for cat in offer.categories.all())
            if preferred_categories
            else False
        ),
    }
...
def update_user_address(user, post_data):
    ...
    street = (
        f"{address_number} {address_street}".strip()
        if address_number
        else address_street
    )
...
def get_filtered_offers(user, filters):
    ...
    # Format offers
    return [
        format_offer_data(offer, today, preferred_categories) for offer in offers_query
    ]
...
def get_offers_stats():
    ...
    return {"total": active_offers.count(), "expiring_soon": expiring_soon}
...
def get_formatted_categories():
    ...
    return [{"value": cat.slug, "name": cat.name} for cat in categories]
...
def create_default_categories():
    ...
    for name, slug in default_categories:
        Category.objects.create(name=name, slug=slug, description=f"{name} services")
